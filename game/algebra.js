...
var categories={
	...
	movies:{
		...
		Q4:{
			...
			Correct:"Kxw M>VCX <WXA<<A	hartoum",
...
$(".StartButton").on("click", function(){
	getQuestion();
	$("body").css("background-image","url('../fond/algebra.jpg')");
	$(".StartPage").hide();})
...
var timer = {
		...
		start: function(){
		    counter=setInterval(timer.count,1000);
		    counter2=setTimeout(timer.timeUp,5000);
		},
...
$(document).ready(function(){
    ...
    $('.answer').click(function(){
								...
								if (!flag){
        $('.answer').css({'background':'url(../cssMathTest/wrong.png) no-repeat right center #F0A74F'});
            $('.right').css({'background':'url(../cssMathTest/right.png) no-repeat right center #F0A74F'});
...
function resulttime(type){
    ...
    $("#result").html('<h1 style="left:500px">'+score +'</h1>');
    $('[data-role="page"]').hide();
    $('#result').slideDown();
    score=0;
}
